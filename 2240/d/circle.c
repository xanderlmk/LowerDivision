...
void BresenhamCircle(int yc, int xc, int rad)
{
	int x=0,y,d;
...
void physics()
{
    ...
    //smallvy -= gravity;
        ...
        {
        smally = yres-10;
        smallvy = -smallvy + 0.50;
            if ( smallvx > 0 )
            {    
                smallvx = smallvx - friction;
            }
            if (smallvx < 0)
            {
                smallvx = smallvx + friction;
...
void render()
{
	...
	char mess[] = "Lab13 Animation 2240 f22";
	XDrawString(dpy, backBuffer, gc, 15, 15, mess, strlen(mess));
	//
	//Draw a large yellow circle
	setColor(255, 255, 0);
    static double angle1 = 0.0;
	double newrad =  (bigradius + sin(angle1)*15); //assembly
	BresenhamCircle(x, y, newrad);
    angle1 += 0.01;
    BresenhamCircle(smallx, smally, 10);
	//
	//==============================================
...
//__asm__(".globl func8\n\t"
  //      ".type func8, @function\n\t"
    ...
    for (i = 0; i<=n; i++) {
        ...
        //double y = func8(angle); //* radius + center[1];
        //y = y * radius + center[1];
        angle = angle + inc;
        setPixel(x, y);
        if (i > 0){
